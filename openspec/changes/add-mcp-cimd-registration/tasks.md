...
- [ ] 1.1 Characterize Doorkeeper 5.6.9's client lookup, authorization grant,
...
- [ ] 1.2 Choose and document the CIMD client representation (controlled
...
- [ ] 1.3 Choose and document the cache store, lower/upper freshness bounds,
  rollout gating (`mcp_server`, a separate flag, or discovery-only), and initial
  domain trust policy before implementation
- [ ] 1.4 Decide whether RFC 9207 can use a stable Doorkeeper 5.6.9 extension or
  requires a separately reviewed dependency upgrade; update `design.md` with the
  verified route

## 2. Metadata retrieval and validation

- [ ] 2.1 Implement one CIMD URL eligibility/parser boundary: absolute HTTPS,
  authority and path present, no userinfo or fragment, with invalid URL-shaped
  client IDs rejected without a network request
- [ ] 2.2 Implement the credential-free metadata fetcher with verified TLS,
  public-address resolution and connection pinning, per-hop redirect
  revalidation, redirect/time/5-KiB response limits, and bounded JSON parsing
- [ ] 2.3 Validate required `client_id`, `client_name`, and `redirect_uris`
  metadata; require exact client-ID and redirect-URI matches; ignore optional
  asset/key URLs rather than fetching them
- [ ] 2.4 Cache only valid documents using exact-URL keys and HTTP freshness
  clamped to the chosen bounds; refetch after expiry and fail closed without
  stale or negative-cache authorization
- [ ] 2.5 Add secrets-safe metrics/logging for cache outcomes, fetch failures,
...
- [ ] 3.1 Route persisted client IDs through the unchanged
  ...
  exchange
- [ ] 3.2 Bind authorization grants and access tokens to the same validated CIMD
  client identity while preserving S256 PKCE, configured scopes, token
  redemption, and revocation behavior
- [ ] 3.3 Update consent for CIMD requests to display validated `client_name`,
  client-ID host, redirect host, and requested scopes before approval
- [ ] 3.4 Return safe OAuth errors for fetch, validation, and redirect failures;
  never redirect an error or success response to an unvalidated URI

## 4. Discovery and authorization-response issuer

- [ ] 4.1 Add RFC 9207 `iss`, equal to the RFC 8414 `issuer`, to every successful
...
- [ ] 4.3 Advertise `client_id_metadata_document_supported: true` only after CIMD
  authorization and token redemption are enabled end to end; retain the DCR
  `registration_endpoint`

## 5. Backward compatibility and documentation

- [ ] 5.1 Keep `POST /oauth/register`, its Rack::Attack throttle, existing DCR
...
- [ ] 5.2 Update `api/docs/mcp_server.md` with CIMD discovery/flow, DCR fallback
...
- [ ] 6.1 Test valid CIMD authorization through code redemption, exact
...
- [ ] 6.2 Test SSRF defenses for private/reserved IPv4 and IPv6, mixed DNS
  answers, DNS rebinding, redirect pivots/downgrades, userinfo, redirect loops,
  timeouts, oversized bodies, and optional metadata URLs that must not be fetched
- [ ] 6.3 Test consent identity, CIMD and RFC 9207 discovery gating, `iss` on
  success and error responses, persisted-client parity, and DCR compatibility
- [ ] 6.4 Run the focused OAuth controller/service tests, full relevant Rails
