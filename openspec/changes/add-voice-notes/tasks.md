...
- [ ] 1.1 Migration: add `voice_id` (string, nullable) to `users`
  (`bin/rails g migration AddVoiceIdToUsers voice_id:string`)
- [ ] 1.2 Confirm no migration is needed for `attachments.transcription_job_id`,
...
- [ ] 1.3 `Attachment#transcript` — plain-text accessor deriving from
  `transcription_vtt` (strip `WEBVTT` header + cue timing lines, join text
  lines); returns `nil` when `transcription_vtt` is blank
- [ ] 1.4 `Attachment#enqueue_transcription_job` — new `after_create_commit`
...
- [ ] 1.5 Model tests: `audio?` attachment enqueues the job and sets
...
- [ ] 2.1 `Stt::WhisperCppClient` — service object wrapping a shelled-out
...
- [ ] 2.2 `TranscribeAttachmentJob` (`Sidekiq::Worker`, `queue: "background"`,
...
- [ ] 2.3 Job test: successful transcription updates the attachment; a
  ...
  in `BaseJob`)
- [ ] 2.4 Document the whisper.cpp binary + model as a deploy prerequisite
...
- [ ] 3.1 `AttachmentSerializer`: add `transcript` (nullable string) and
  `transcription_job_status` (nullable string) fields
- [ ] 3.2 `McpTools::GetAttachmentTranscript` — org-scoped MCP tool, resolves
...
- [ ] 3.3 Register `get_attachment_transcript` in `McpToolRegistry`
- [ ] 3.4 Tests: `read_post`/`read_note`/`read_messages` responses include
...
- [ ] 4.4 Extract `AttachmentUploader.put_and_attach!(subject:, bytes:,
...
- [ ] 4.5 `McpTools::SpeakReply` — org-scoped MCP tool, input `text` +
...
- [ ] 4.6 Register `speak_reply` in `McpToolRegistry`
- [ ] 4.7 `User#voice_id` — `speak_reply` defaults `voice_id` to
  `Current.user.voice_id` when the caller omits it
- [ ] 4.8 Tests: `speak_reply` with default (Edge) provider creates an audio
...
- [ ] 4.9 Update `api/docs/mcp_server.md` with the new tool and the
  `voice_id` field on `User`

## 5. Quality gates and close-out

- [ ] 5.1 `bundle exec rubocop -A` on all new/changed Ruby files
- [ ] 5.2 `bin/rails test` for the full changed-file set (models, jobs, mcp
  tools, serializers)
- [ ] 5.3 `bin/rails db:migrate` locally to confirm the `voice_id` migration
  applies cleanly; update `api/db/schema.rb`
- [ ] 5.4 Update `api/README.md`/deploy docs with the whisper.cpp + edge-tts
  binary prerequisites (single consolidated section, cross-referenced from
  tasks 2.4 and 4.2)
- [ ] 5.5 `openspec validate add-voice-notes --strict` passes
- [ ] 5.6 If tracked in beads, file/close the relevant `bd` issue(s) for this
