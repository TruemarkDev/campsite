...
import { SITE_URL, WEB_URL } from '@campsite/config'
import { getNoteExtensions } from '@campsite/editor'
import { PublicNote } from '@campsite/types'
import { Button, Link, Logo, UIText } from '@campsite/ui'
        ...
        <div className='mx-auto flex w-full max-w-[44rem] flex-1 flex-col gap-4 px-4 pb-4 md:px-6 lg:px-0'>
          <div className='flex flex-col gap-3 py-8 select-text md:py-14 lg:py-20'>
            <UIText
              element='h1'
              className={cn(
                '-mx-px mb-1 w-full border-0 bg-transparent p-0 text-[clamp(2rem,_4vw,_2.5rem)] leading-[1.2] font-bold outline-none focus:border-0 focus:ring-0 focus:outline-none'
              )}
            >
              {note.title || 'Untitled'}
            </UIText>
