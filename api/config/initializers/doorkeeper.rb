...
module Doorkeeper
  module SecretStoring
    # PDKDF2+SHA512 secret storing strategy adapted from GitLab.
    # https://gitlab.com/gitlab-org/gitlab-foss/-/blob/master/lib/gitlab/doorkeeper_secret_storing/secret/pbkdf2_sha512.rb
    class Pbkdf2Sha512 < ::Doorkeeper::SecretStoring::Base
      ...
      def self.transform_secret(plain_secret)
        digest(plain_secret, STRETCHES, SALT)
      end

      private_class_method def self.digest(password, stretches, salt)
      ...
      private_class_method def self.pbkdf2_checksum(hash, password, stretches, salt)
        raise "Stretches must be greater than zero" if stretches.to_i <= 0
...
Doorkeeper.configure do
  ...
  # These attributes are included in the AccessGrant form so that we can assign tokens
  # to polymorphic resource owners.
  #
  custom_access_token_attributes [:resource_owner_type, :resource_owner_id]
  ...
  #
  hash_token_secrets using: "::Doorkeeper::SecretStoring::Pbkdf2Sha512", fallback: "::Doorkeeper::SecretStoring::Sha256Hash"
  ...
  #
  force_ssl_in_redirect_uri { |uri| uri.host != "app.campsite.dev" }
